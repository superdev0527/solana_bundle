...
import fs from 'fs';
import { ACCOUNT_SIZE, AccountLayout, MintLayout, NATIVE_MINT, TOKEN_PROGRAM_ID, amountToUiAmount, createInitializeAccountInstruction, decodeAmountToUiAmountInstructionUnchecked, getAssociatedTokenAddressSync, getOrCreateAssociatedTokenAccount } from "@solana/spl-token";
...
import { calcDecimalValue, calcNonDecimalValue, getNullableResutFromPromise } from "./utils";
...
export class BaseRay {
  ...
  async buyFromPool(input: BuyFromPoolInput): Promise<{ ixs: web3.TransactionInstruction[], signers: web3.Signer[] }> {
    this.reInit();
    const { amountIn, amountOut, poolKeys, user, fixedSide, tokenAccountIn, tokenAccountOut, skipAtaInit } = input
  ...
  async createPool(input: CreatePoolInput, user: web3.PublicKey) {
    ...
    if (input.baseMint.toBase58() != NATIVE_MINT.toBase58()) {
      const requiredFund = Number(baseAmount.toString())
      const balance = Number(AccountLayout.decode(userBaseAtaInfo!.data).amount.toString())
      if (balance < requiredFund) throw "Not Enough fund"
    }
    if (input.quoteMint.toBase58() != NATIVE_MINT.toBase58()) {
      const requiredFund = Number(quoteAmount.toString())
      const balance = Number(AccountLayout.decode(userQuoteAtaInfo!.data).amount.toString())
      if (balance < requiredFund) throw "Not Enough fund"
    }
    ...
    return { ixs, signers, poolId: Liquidity.getAssociatedId({ marketId: marketInfo.marketId, programId: this.ammProgramId }), baseAmount, quoteAmount }
  }
  ...
  async computeBuyAmount(input: ComputeBuyAmountInput, etc?: { extraBaseResever?: number, extraQuoteReserve?: number, extraLpSupply?: number }) {
    ...
    if (inputAmountType == 'send') {
      ...
      if (buyToken == 'base') {
        ...
        amountOut = Liquidity.computeAmountOut({ amountIn, currencyOut: baseR, poolInfo, poolKeys, slippage: new Percent(0) }).minAmountOut as TokenAmount
        const OAmount = calculateOutputAmount({ inputAmount: Number(amountIn.raw.toString()), inputReserve: Number(quoteReserve.toString()), outputReserve: Number(baseReserve.toString()) })
        const Osdf = OAmount - Math.trunc((OAmount * 0.0022))
        log({ OAmount, Osdf, cOut: Number(amountOut.raw.toString()) })
      } else {
