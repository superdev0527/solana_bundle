...
export class BaseMpl {
  ...
  async createToken(input: CreateNftBuilderParams, opts: { decimal?: number, mintAmount?: number, mintKeypair?: web3.Keypair }) {
    ...
    try {
      ...
      ixs.push(web3.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: 400_000 }))
      ixs.push(...mintIxs)
      ...
      // const res = await this.connection.sendRawTransaction(tx.serialize(), { skipPreflight: true })
      return {
        txSignature: res,
